...
class HAL(ContinualModel):
    ...
    def get_anchors(self, dataset):
        ...
        for _ in range(self.finetuning_epochs):
            ...
            out, feats = self.spare_model(inputs)
        ...
        for a_class in classes_for_this_task:
            ...
            for i in range(self.anchor_optimization_steps):
                ...
                loss = -torch.sum(self.loss(self.spare_model(e_t.unsqueeze(0), returnt='out'), torch.tensor([a_class]).to(self.device)))
                loss.backward()
                cum_loss += loss.item()
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if len(self.anchors) > 0:
            ...
            with torch.no_grad():
                pred_anchors = self.net(self.anchors, returnt='out')
